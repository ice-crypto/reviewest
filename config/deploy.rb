...
# デプロイするアプリケーション名
set :application, 'reviewest'

# cloneするgitのレポジトリ
# 1-3で設定したリモートリポジトリのurl
set :repo_url, 'git@github.com:ice-crypto/reviewest.git'
set :ssh_options, {
  forward_agent: true,
  user: fetch(:user),
...
# deploy先のディレクトリ。
set :deploy_to, '/var/www/reviewest/'
      ...
      with rails_env: fetch(:rails_env) do
        within current_path do
                ...
                # データベース名はdatabase.ymlに設定した名前で
                  sql = "CREATE DATABASE IF NOT EXISTS reviewest_production;"
                  # クエリの実行。
